...
def beginextensible_at(objidd):
    """return the index of the field that is the beginning of the extensible fields.
    In the idd, this is tagged by begin-extensible"""
    extensibleval = hasextensible(objidd)
    if extensibleval:
        for i, idditem in enumerate(objidd):
            if idditem.has_key(u'begin-extensible'):
                return i
                break
    return None
    
def extensiblelist(idfobject):
    """return the list that has the items in the extensible list"""
    pass    