...
from eppy import iddhelpers

def test_hasextensible():
    """py.test for hasextensible"""
    tdata = (([{u'extensible:2': [u'- repeat last two fields, remembering to remove ; from "inner" fields.'],
  u'group': u'Schedules',
...
([{u'extensible:1': [u''],
  ...
  u'memo': [u'This object is used to control details of the solar, shading, and daylighting models'],
  u'unique-object': [u'']},
 {u'default': [u'AverageOverDaysInFrequency'],
  u'field': [u'Calculation Method'],
  u'key': [u'AverageOverDaysInFrequency', u'TimestepFrequency'],
  u'note': [u'choose calculation method. note that TimestepFrequency is only needed for certain cases',
...
([{u'group': u'Simulation Parameters',
  ...
  u'required-object': [u''],
  u'unique-object': [u'']},
 {u'default': [u'NONE'], u'field': [u'Name'], u'retaincase': [u'']}], None), # iddobj, expected
    )
...
def test_beginextensible_at():
 ...
 {u'default': [u'No'],
  ...
  u'note': [u'when the interval does not match the user specified timestep a Average choice will average between the intervals request (to',
   u'timestep resolution. A No choice will use the interval value at the simulation timestep without regard to if it matches',
   u'the boundary or not. A Linear choice will interpolate linearly between successive values.'],
  u'type': [u'choice']},
...
([{u'group': u'Simulation Parameters',
  ...
  u'required-object': [u''],
  u'unique-object': [u'']},
 {u'default': [u'NONE'], u'field': [u'Name'], u'retaincase': [u'']}], None), # objidd, expected
    ) 
    for objidd, expected in tdata:
        result = iddhelpers.beginextensible_at(objidd)
        assert result == expected
        
def test_extensiblelist():
    """py.test for extensiblelist"""
    data = ((), # idfsnippet, expected
    )        